...
#add a comma to run multiple files
...
def process_file(file_name, include_statics):
    if "." in file_name:
...
def prompt_file():
    file_name_input = input(f"{name_prefix}Enter the file name: ")
    if file_name_input == "-c":
        configure()
    elif file_name_input == "-e":
        exit()
    else:
        if " -s" in file_name_input:
            file_name = file_name_input.replace(" -s", "")
            process_file(file_name, include_statics=True)
        else:
            file_name = file_name_input
            process_file(file_name, include_statics=False)
